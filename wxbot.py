...
class WXBot:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            msg_content['video'] = video
            if self.DEBUG:
                print '    %s[video] %s' % (msg_prefix,video)
        else:
            msg_content['type'] = 99
            msg_content['data'] = content
            if self.DEBUG:
    ...
    def createchartroom(self, members):
        url = self.base_uri + '/webwxcreatechatroom?r=%i&pass_ticket=%s'% (int(time.time()), self.pass_ticket)
        params = {
            'BaseRequest': self.base_request,
